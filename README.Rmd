...
- **Re-running top-level lines** -- at top level (e.g. re-running selected
  ...
  distinct.
- **Run digests** -- `logtree_summary(filter = NULL, depth = NULL)` reports a
  breadcrumb digest of errors, warnings, and pinned leaves once a run ends
  (`with_logging()` calls it automatically unless `summary = FALSE`);
  `filter` restricts by status, `depth` trims each breadcrumb to its `N`
  deepest nodes.
