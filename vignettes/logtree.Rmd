...
Two arguments control horizontal space, independently of any glyph. `compact`
sets the per-level tree column: `"medium"` drops the trailing gap after each
connector, `"tight"` also slims the connectors to one character. `glyph_gap`
sets the space between a line's status glyph and its message -- `1` by
default, `0` for the tightest look, `2` or more for a roomier message column.
...
Both ride the active theme and are cleared by the next preset swap
(`logtree_theme("unicode")`). Neither reaches file sinks, which render through
the ascii preset with its built-in spacing.
