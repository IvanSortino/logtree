...
`compact` and `glyph_gap` control horizontal space, independently of any glyph.
`compact` sets the per-level tree column: `"medium"` drops the trailing gap
after each connector, `"tight"` also slims the connectors to one character.
`glyph_gap` sets the space between a line's status glyph and its message -- `1`
by default, `0` for the tightest look, `2` or more for a roomier message
column.
...
These two and `wrap` are *scalars* carried on the theme, not override slots:
they are set through `logtree_theme()`'s own arguments rather than through
`overrides`. All three ride the active theme and are cleared by the next preset
