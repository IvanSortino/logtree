...
`log_step(label, group_by = c(name = value))` collapses adjacent steps that
share the same `value` under one synthetic header, instead of nesting each
step on its own. The group stays open across calls with a matching value
and only closes -- as a sibling, with its own status aggregated from its
members -- once a step with a different value (or a plain ungrouped step)
appears at the same level. Grouping is strictly adjacency-based: the same
value recurring later, after something else has closed the group, opens a
fresh one rather than reusing the old header.
...
for (file in c("data1.csv", "data1.csv", "data2.csv")) {
  process_file(file)
}
```
...
through `logtree` without rewriting them. `logtree` provides a custom layout
function `layout_logtree` that integrates with `logger`'s pipeline. Pair it
with `logger::appender_void` (a ready-made no-op appender) so that `logtree`
does the rendering as a side effect.
...
# Restore the namespace's original layout/appender
