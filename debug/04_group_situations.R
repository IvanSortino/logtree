...
run_section("A. adjacent same value -> one header", function() {
  check <- function(val, name) log_step(name, group = c(batch = val))
  log_step("Root A")
  check("x", "step 1")
  check("x", "step 2")
})
...
run_section("B. value changes each iteration -> header per item", function() {
  proc <- function(i) log_step(paste0("item ", i), group = c(iter = i))
  log_step("Root B")
  for (i in 1:3) proc(i)
})
...
run_section("D. non-adjacent same value -> separate groups", function() {
  step <- function(v, name) log_step(name, group = c(k = v))
  log_step("Root D")
  step("a", "a1")
...
run_section("E. plain step after a group -> sibling, group closes", function() {
  g <- function() log_step("grouped", group = c(k = 1))
...
run_section("G. nested groups (group inside a grouped member)", function() {
  outer <- function(i) {
    log_step(paste0("outer ", i), group = c(run = i))
    chk <- function(name, cat) log_step(name, group = c(phase = cat))
...
# ---------------------------------------------------------------------------
# H. Unnamed group -> the header name falls back to as.character(value).
run_section("H. unnamed group -> name = value", function() {
  step <- function(v) log_step(paste0("do ", v), group = v)
  log_step("Root H")
...
run_section("I. warning elevation inside a grouped member", function() {
  step <- function(i) {
    log_step(paste0("task ", i), group = c(run = 1))
    log_info("working")
...
run_section("J. error thrown inside a grouped step (with_logging)", function() {
  step <- function(i) {
    log_step(paste0("job ", i), group = c(run = 1))
...
# ---------------------------------------------------------------------------
# K. group must be length-1 (named vector). Longer input errors early.
section("K. invalid group (length > 1) errors")
logtree_reset()
tryCatch(
  (function() log_step("x", group = c(a = 1, b = 2)))(),
